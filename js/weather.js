...
function addWeather(arrayMaxTemp, arrayMinTemp, iconsrc, date) {
  ...
  forecastBox.innerHTML = `<div class="day_box">
    <h2>${getDayofWeek(date.getDay())}</h2>
    <h3 id="date"> ${date.getMonth() + 1}/${date.getDate()}</h3>
        <img src="${iconsrc[0]}" class = "fiveDayIcon" alt="Weather icon Day 1">
        <p class="highLow"><span>${arrayMaxTemp[0]}</span> &deg;F / <span>${
    arrayMinTemp[0]
  }</span> &deg;F</p>
    </div>

    <div class="day_box">
    <h2>${getDayofWeek(date.getDay() + 1)}</h2>
        <h3 id="date">${date.getMonth() + 1}/${date.getDate() + 1} </h3>
        <img src="${iconsrc[1]}" class = "fiveDayIcon" alt="Weather icon Day 2">
        <p class="highLow"><span>${arrayMaxTemp[1]}</span> &deg;F / <span>${
    arrayMinTemp[1]
  }</span> &deg;F</p>
    </div>

    <div class="day_box">
        <h2>${getDayofWeek(date.getDay() + 2)}</h2>
        <h3 id="date">${date.getMonth() + 1}/${date.getDate() + 2} </h3>
        <img src="${iconsrc[2]}" class = "fiveDayIcon" alt="Weather icon Day 3">
        <p class="highLow"><span>${arrayMaxTemp[2]}</span> &deg;F / <span>${
    arrayMinTemp[2]
  }</span> &deg;F</p>
    </div>

    <div class="day_box">
        <h2>${getDayofWeek(date.getDay() + 3)}</h2>
        <h3 id="date">${date.getMonth() + 1}/${date.getDate() + 3} </h3>
        <img src="${iconsrc[3]}" class = "fiveDayIcon" alt="Weather icon Day 4">
        <p class="highLow"><span>${arrayMaxTemp[3]}</span> &deg;F / <span>${
    arrayMinTemp[3]
  }</span> &deg;F</p>
    </div>
 
    <div class="day_box">
    <h2>${getDayofWeek(date.getDay() + 4)}</h2>
        <h3 id="date">${date.getMonth() + 1}/${date.getDate() + 4} </h3>
        <img src="${iconsrc[4]}" class = "fiveDayIcon" alt="Weather icon Day 5">
        <p class="highLow"><span>${arrayMaxTemp[4]}</span> &deg;F / <span>${
    arrayMinTemp[4]
  }</span> &deg;F</p>
    </div>`;
}
...
/*****************************************
 * Get Day of the Week
 * From a given number, return a string with
 * the day of the week.
 * ***************************************/
